 ...
 */
package sistema.accesoadatos;

import java.util.*;
import java.sql.*;
import sistema.entidadesdenegocio.Cliente;

public class ClienteDAL {

    static String obtenerCampos() {
        return "c.Id, c.Nombre, c.Apellido, c.Telefono, c.Direccion";
    }
    ...
    public static int modificar(Cliente pCliente) throws Exception {
        ...
        String sql;
        try (Connection conn = ComunDB.obtenerConexion();) {
            sql = "UPDATE Cliente SET Nombre=?, Apellido = ?, Telefono = ?, Direccion = ? WHERE Id=?";
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) {
                ps.setString(1, pCliente.getNombre());
                ps.setString(2, pCliente.getApellido());
                ps.setString(3, pCliente.getTelefono());
